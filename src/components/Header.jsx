  ...
  return (
    <div className='absolute w-full px-8 py-2 bg-gradient-to-b from-black z-10 flex justify-between'>
      ...
      {user && <div className='flex p-2'>
        <img className='h-10 w-10 m-2' src={user?.photoURL} alt="user-icon" />
        <button onClick={handleSignOut} className='font-bold text-white p-2 m-2 bg-red-500 rounded-lg cursor-pointer'>Sign Out</button>
      </div>}
    </div>
  )
}
